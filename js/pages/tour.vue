<template>
    <div class="wrapper shadow-lg">
        ...
        <div v-show="! loading" class="bg-white right-side" style="overflow: auto" ref="formContainer">
            ...
            <transition :name="formTransition" mode="out-in">
                <stop-form v-if="mode == 'stop'" 
                    ref="stopForm"
                    :isEndPoint="currentStop.id && currentStop.id === tour.end_point_id"
                    :isStartPoint="currentStop.id && currentStop.id === tour.start_point_id"
                    @addStop="createStop()"
                ...
                <tour-form v-if="mode == 'tour' && tour.id"
                    ref="tourForm"
                    @publish="publish"
                    @publishWithConfirm="publishWithConfirm"
                    />
            </transition>
        </div>
            ...
            <transition name="fade" mode="out-in">
                    ...
                    <div class="f-1">
                        <draggable :list="tour.stops" @change="saveStopOrder" class="stop-list">
                            <stop-card v-for="item in tour.stops"
                                :key="item.id" 
                                :stop="item"
                                :isEndPoint="item.id && item.id === tour.end_point_id"
                                :isStartPoint="item.id && item.id === tour.start_point_id"
                                :tourID="tour.id"
                ...
                <div v-else class="bg-gray h-100 p-relative" key="map">
                    <div class="map-toolbar">
                        ...
                        <b-btn v-if="tour.type == 'adventure'" variant="secondary" class="d-inline" @click="toggleShowConnections()">
                            <span v-if="connectionMode == 'hide'"><fa :icon="['fas', 'eye']" />&nbsp; Show Connections</span>
                            <span v-else><fa :icon="['fas', 'eye-slash']" />&nbsp;Hide Connections</span>
                        </b-btn>
                        ...
                        <div v-if="tour.type == 'outdoor'" class="d-inline">
                            ...
                            <b-btn v-if="routeMode != 'edit'" variant="secondary" class="d-inline" @click="deleteRoute()">
                                <fa :icon="['fas', 'trash']" />&nbsp;Delete Route
                            </b-btn>
                    ...
                    <tour-map @clickStop="showStopForm" @clickTour="showTourForm()" @saveChangedStopForm="saveChangedStopForm"/>
                </div>
            </transition>
        </div>
        ...
        <confirm-modal ref="confirmPublish" yesButton="Submit">
            Before you submit your junket, you must ensure that it is complete and ready for use. 
            This means that you should follow the guidelines for a finished Junket. 
            Every stop must contain at least one quality image, include complete text, and audio. 
            The route of your junket must complete and ordered. 
            Junket will not approve experiences that do not deliver quality experiences.
...
export default {
    ...
    computed: {
        ...
        ...mapGetters({
            ...
            connectionMode: 'routes/connectionMode',
    ...
    methods: {
        ...
        async saveChangedStopForm() {
            await this.$refs.stopForm.save();
        },
        ...
        createStop(location = {}, confirmed = false) {
            if (this.tour.allowed_stop_count <= 0) {
                alerts.addMessage('error', "You have reached your maximum number of allowed stops.");
                return;
            }
            EventBus.$emit('stop-audio');
        ...
        saveRoute() {
            if (this.hideMode) {
                this.$store.commit('tours/setTourRoute', this.tour.route);
        ...
        toggleShowConnections() {
            if (this.connectionMode == 'show') {
                this.$store.commit('routes/setConnectionMode', 'hide');
            } else {
                this.$store.commit('routes/setConnectionMode', 'show');
            }
        },
        ...
        setStopViewMode(mode) {
            this.$store.commit('tours/setIsInitialMap', false);
        ...
        publishWithConfirm() {
            this.$refs.confirmPublish.confirm(() => {
                this.publish();
            });
        },
    },
    ...
    async mounted() {
        ...
        this.loading = false;
        this.$store.commit('tours/setIsInitialMap', true);
    },
