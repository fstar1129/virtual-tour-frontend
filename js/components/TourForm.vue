<template>
    <div>
        <div v-show="hasTour">
            ...
            <div v-if="! form.main_image_id" class="feature-box empty" @click.stop="openFileDialog('main_image')" v-b-tooltip.hover titile="Images must be at least 700 x 400 pixels, in landscape orientation, and in high resolution for image quality.">
                <div class="addlink">
            ...
            <div class="circbox"
                ...
                <div class="addrescir">
                    ...
                    <span v-if="! form.pin_image" class="info-icon map-icon" v-b-tooltip.hover title="Images must be at least 164 x 164 pixels, and in a transparent format like png">
                        <div class="j-icon">j</div>
                    </span>
        ...
        <div class="p-3">
            ...
            <div v-if="isAdmin && tour.status == 'pending'" class="d-flex mb-4">
                ...
                <busy-button class="ml-3" variant="danger" size="sm" :disabled="busy" @click.prevent="unpublishByRejection()">Reject</busy-button>
            </div>
            
            <div v-if="hasTour">
                <h4 style="line-height: 26px;">
                    Description
                    <span class="info-icon" v-b-tooltip.hover title="The description should have a minimum of 100 words. Also, if you are using photos from the public domain, please ensure to include a photo credit at the bottom of your content along with a link to the source.">
                        <div class="j-icon mgt-3">j</div>
                    </span>
                </h4>
                <b-form-group>
            ...
            <b-form-group>
                <b-form-select 
                    ...
                    <option v-if="user.plan_name !== 'starter'" value="free">Free</option>
                    <option value="premium">Paid</option>
                </b-form-select>
                <input-help :form="form" field="pricing_type" text=""></input-help>
            </b-form-group>

            <b-form-group v-if="this.isAdmin && !this.isAdmin && form.pricing_type == 'premium'">
                <h4>In-App ID</h4>
            ...
            <h4 class="info-heading" v-if="form.pricing_type == 'premium'">
                Price
                <span class="info-icon" v-b-tooltip.hover title="Please set the price you would like to charge for this Junket. Price is determined by number of tokens. One token is one US Dollar.">
                    <div class="j-icon mgt-3">j</div>
                </span>
            </h4>
            <div class="price-view" v-if="form.pricing_type == 'premium'" >
                <b-form-group class="token-cost">
                    <b-input-group :prepend="currencySymbol">
                        <b-form-input id="local_token_cost"
                            :disabled="busy"
                            type="number"
                            v-model="form.local_token_cost"
                            placeholder="Price"  class="token-cost-input">
                        </b-form-input>
                    </b-input-group>
                    <input-help :form="form" field="local_token_cost" text=""></input-help>
                </b-form-group>
                <b-form-group class="currency-selector">
                    <b-form-select :disabled="busy" v-model="form.currency" class="currency-option">
                        <option v-for="item in Object.entries(rates)" 
                            :key="item[0]"
                            :value="item[0]"
                            v-text="item[0]"
                        ></option>
                    </b-form-select>
                    <input-help :form="form" field="currency" text=""></input-help>
                </b-form-group>
            </div>
            ...
            <b-form-group>
                <b-form-select 
                    ...
                    <option v-if="user.plan_name !== 'starter'" value="adventure">Adventure</option>
                </b-form-select>
            ...
            <div v-if="hasTour">
                ...
                <!-- PRIZE -->
                <div v-if="user.plan_name !== 'starter'">
                    <h4 class="mt-3">
                        Trophy
                        <span class="info-icon" v-b-tooltip.hover :title="`Trophies are awarded when 80% of a tour has been physically visited using the trigger radius, or when an adventure has been completed fast enough. Trophies are awarded automatically. You can customize the appearance of the Trophy for your Junket by uploading an image.`">
                            <div class="j-icon">j</div>
                        </span>
                    </h4>
                    ...
                    <div class="form-check mt-3 mb-3">
                        <div class="custom-control custom-checkbox mr-sm-2">
                            ...
                            <span v-if="form.has_prize" class="info-icon" v-b-tooltip.hover :title="`Prizes are highly recommended but optional, and will only be made available once the user has earned a trophy. Prizes can be awarded only once. You determine how long the user has to redeem their prize, where and how to redeem it, and any additional details.`">
                                <div class="j-icon">j</div>
                            </span>
                        </div>
                    </div>
                ...
                <div v-if="isAdmin">
                    <h4 class="mt-3">
                        CREATE PROMO CODE
                        <span class="info-icon" v-b-tooltip.hover :title="`Only two codes may be created.`">
                            <div class="j-icon">j</div>
                        </span>
                    </h4>
                    <div class="form-check mt-3 mb-3">
                        <div class="custom-control custom-checkbox mr-sm-2">
                            <input type="checkbox" class="custom-control-input" id="has_promo_code" v-model="has_promo_code">
                            <label class="custom-control-label" for="has_promo_code">Enable Promo Code</label>
                        </div>
                    </div>
                    <div v-show="has_promo_code">
                        <b-row>
                            <b-col lg="6">
                                <b-form-group>
                                    ...
                                    <input-help :form="form" field="discount" text=""></input-help>
                                </b-form-group>
                            </b-col>
                            <b-col lg="6">
                                <b-form-group class="mg-0 quantity-input">
                                    <b-input-group>
                        ...
                        <b-row>
                            ...
                            <b-col lg="6">
                                <div class="custom-control custom-checkbox mr-sm-2">
                                    <input type="checkbox" class="custom-control-input" id="unlimited" v-model="form.unlimited">
                        ...
                        <b-form-group>
                            <b-input-group prepend="PROMO CODE:" class="prepend-design mt-3">
                                <b-form-input id="promo_code"
                                    :disabled="busy"
                                    type="text"
                            ...
                            <input-help :form="form" field="promo_code" text=""></input-help>
                            <p class="show-promo-info">
                                Your promo code must consist of 4-6 characters.
                            </p>
                        </b-form-group>
                        <b-row>
                            <b-col lg="12">
                        ...
                        <p v-for="promoCode in tour.promo_codes" v-show="isSavedPromoCode" class="show-promo-detail" :key="promoCode.id">
                            Discount: {{ promoCode.discount }}%, Quantity: {{ promoCode.quantity }}, Code: {{ promoCode.promo_code }}
                        </p>
                    </div>
                </div>
                <!-- <h4 class="mt-3">
                    SELECT LANGUAGES
                    <span class="info-icon" v-b-tooltip.hover :title="`Select language/s to apply to Junket.`">
                        <div class="j-icon">j</div>
                    </span>
                </h4>
                <b-form-group>
                    <b-form-select :disabled="busy" v-model="language">
                        <option value="">-- Select Languages to apply to Junket --</option>
                        <option v-for="item in tour.languages" 
                            :key="item.id" 
                            :value="item.id"
                            v-text="item.text"
                        ></option>
                ...
                <b-row>
                    <b-col lg="12" class="mt-3">
                        <busy-button 
                            ...
                            variant="primary" 
                            class="w-100" 
                            @click.prevent="publishWithConfirm()"
                            :disabled="busy"
                            :hoverTitle="`Your Junket will not become live until it is approved by the Junket team for quality assurance. Please make sure that your junket is complete that all stops have quality images, audio, and text.`"
                        >
                            Submit For Approval
                        </busy-button>
                    </b-col>
            ...
            <div v-if="isAdmin">
                ...
                <b-form-group>
                    <b-form-textarea id="description"
                        ...
                        required
                        rows="5"
                        placeholder="The reasons why it is rejected">
                    </b-form-textarea>
                </b-form-group>
            </div>
            <div v-else>
...
export default {
    ...
    computed: {
        ...
        ...mapGetters({
            ...
            savePromoCodeUrl: "tours/savePromoCodeUrl",
    ...
    methods: {
        submit() {
            ...
            var val = this.form.local_token_cost / this.rates[this.form.currency]
            this.form.token_cost = Math.ceil(val);
        ...
        async unpublishByRejection(confirmed = false) {
            if (! confirmed && this.tour.is_awaiting_approval) {
                return this.$refs.confirmCancelApproval.confirm(() => {
                    this.unpublishByRejection(true);
                });
            } else if (! confirmed) {
                return this.$refs.confirmUnpublish.confirm(() => {
                    this.unpublishByRejection(true);
            ...
            axios.put(this.saveUrl + '/unpublish-by-rejection', {reason: this.rejectDescription})
                .then(response => {
                    this.$store.commit('tours/setCurrent', response.data.data);
                    alerts.addMessage('success', response.data.message);
                    this.busyPublishing = false;
        ...
        //     for ( var i = 0; i < this.promoCodeLen; i++ ) {
        //         result += characters.charAt(Math.floor(Math.random() * charactersLength));
        //     }
        //     this.form.promo_code = result;
        // },
        ...
        savePromoCode() {
            var promo_form = {
                ...
                quantity: this.form.unlimited ? 999 : this.form.quantity,
                promo_code_limit: this.form.promo_code_limit - 1,
            }
            let url = this.savePromoCodeUrl;
            let method = "post";
            new Form(promo_form).submit(method, url).then(({ data }) => {
                    this.isSavedPromoCode = true;
                    this.$store.commit("tours/setCurrent", data.data);
    ...
    async mounted() {
        ...
        await Vue.nextTick();
        this.markFormAsChanged(false);
        this.rates = await ex.exchangeRates().latest()
                .base(ex.currencies.USD)
                .fetch();
        this.currencySymbol = getSymbolFromCurrency(this.form.currency);
        console.log(this.tour);
        this.isSavedPromoCode = this.tour.promo_codes.length > 0;
    },
    ...
    watch: {
        ...
        'form.currency': function(newVal, oldVal) {
            this.currencySymbol = getSymbolFromCurrency(newVal);
        }
    }
};
</script>
...
.prepend-design .input-group-text {
