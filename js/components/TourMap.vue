...
export default {
    ...
    computed: {
        ...mapGetters({
            ...
            mapPosition: 'stops/mapPosition',
            isInitialMap: 'tours/isInitialMap'
        }),
        ...mapState({
        ...
        pinIcon() {
            if (! this.tour.pin_image_id) {
                return {
                    url: 'https://s3.amazonaws.com/cms.wejunket.com/images/pin-icon.png',
    ...
    methods: {
        ...
        createMarker(stop, isCurrent = false) {
            ...
            if (stop.id !== null) {
                marker.pin.addListener('mouseover', () => {
                    tempInfoWindow = this.getInfoWindow(stop);
                    tempInfoWindow.open(this.$refs.map, marker.pin)
                });
                marker.pin.addListener('mouseout', () => tempInfoWindow.close());
            }
        ...
        zoomToFitMarkers(initial = false) {
            ...
            if (markers[0] && markers[0].pin !== null) {
        ...
        initMap(element) {
            // center over the usa
            let defaultPosition = this.mapPosition;
            // center over tour
            if (this.isInitialMap && this.tourLocation) {
            ...
            google.maps.event.addListener(this.map, "center_changed", this.storeMapPostion);

            this.loadTourMarker();
            this.loadStopMarkers();
            if (this.isInitialMap) {
        ...
        async onDragMarker(event, marker, stop) {
            ...
            // When drag is ended, it will auto save the updated location after 500(Not sure this is fine for all browsers and computers) ms.
            setTimeout(() => {
                this.$emit('saveChangedStopForm');
            }, 500);
        },

        storeMapPostion(event) {
            var zoom = this.map.getZoom();
            var center = this.map.getCenter();
            this.$store.commit('stops/setMapPosition', {
                zoom: zoom,
                center: {
                    lat: center.lat(),
                    lng: center.lng()
        ...
        drawConnectedStops() {
            this.tour.connections.forEach(stops => {
                ...
                this.connectionLine.push(line);
            })
        },

        hideConnectedStops() {
            this.connectionLine.forEach(line => {
                line.setMap(null);
            })
        },
    ...
    watch: {
        ...
        connectionMode(newVal) {
            if (newVal == 'hide') {
                this.hideConnectedStops();
            } else {
                this.drawConnectedStops();
            }
        },
